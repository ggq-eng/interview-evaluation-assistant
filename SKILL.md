---
name: interview-evaluation-assistant
description: 构建 AI 面试评估助手网页工具。当用户需要创建用于 HR 面试场景的评估工具时使用，包括：岗位 JD
  解析生成评估维度、简历智能评分、面试记录与结构化报告生成、多候选人横向对比等功能。触发词：面试评估、HR工具、简历评分、候选人对比、面试报告、招聘助手。输出为单一
  HTML 文件，前端实现，支持离线关键词匹配与可选的在线 AI 语义模式（调用大模型进行真实人岗匹配理解），使用 localStorage 存储数据。
disable: false
---

# AI 面试评估助手 —— 网页工具构建 Skill
